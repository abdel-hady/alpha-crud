...
export class ProductsService {
  ...
  async getAllProducts(
    ...
    if (cachedProducts) {
      this.logger.log(`Retrieved products from cache for key: ${cacheKey}`);
    ...
    this.logger.log(`Retrieved ${totalItems} products from the database`);
    ...
    this.logger.log(`Cached products data for key: ${cacheKey}`);
  ...
  async updateProductStockInQueue(productId: number) {
    await this.productQueue.add('update-product-stock', { productId });
    this.logger.log(
      `Stock update job added to queue for product ID: ${productId}`,
    );
  }
  async updateProduct(
    id: number,
